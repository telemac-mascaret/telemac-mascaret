...
!-----------------------------------------------------------------------
     ...
     &           0)
      ...
      CALL BIEF_ALLVEC(1,EVOL_M, 'EVOL_M', IELMT, 1, 2,MESH) ! TOTAL MASS EVOLUTION DUE TO BEDLOAD
...
!
      ...
      ALLOCATE(MASS_SAND(NSAND,NOMBLAY,NPOIN)) ! FRACTION OF EACH CLASS FOR EACH LAYER
      ...
      ALLOCATE(TOCE_SAND(NSAND))
      ALLOCATE(TOCE_MUD(NOMBLAY,NPOIN))
      ALLOCATE(XKV(NOMBLAY,NPOIN))	  
!
      ...
      CALL ALLBLO(RATIO  , 'RATIO ') ! MASS FRACTION OF EACH CLASS (FOR THE TWO FIRST LAYERS??)
...
!
      ...
      CALL ALLBLO(EVCL_M, 'EVCL_M') ! MASS EVOLUTION FOR EACH CLASS DUE TO BEDLOAD TRANSPORT
!
...
!
      CALL BIEF_ALLVEC_IN_BLOCK(RATIO,NOMBLAY*NSAND,
     &                          1,'RATIO ',0,1,0,MESH)
      DO K=1,NOMBLAY
        DO I=1,NSAND
          RATIO%ADR(K+(I-1)*NOMBLAY)%P%R=>RATIO_SAND(I,K,1:NPOIN)
          RATIO%ADR(K+(I-1)*NOMBLAY)%P%MAXDIM1=NPOIN
          RATIO%ADR(K+(I-1)*NOMBLAY)%P%DIM1=NPOIN
        ENDDO
      ENDDO
!
...
!
      ...
      CALL BIEF_ALLVEC_IN_BLOCK(EVCL_M,NSICLA,1,'EVCL_M',IELMT,1,2,MESH)
...
! JWI END
!     RATIO: FROM 23 TO 22+NOMBLAY*NSICLA
!
      DO I = 1,NOMBLAY*NSICLA
        CALL ADDBLO(VARSOR, RATIO%ADR(I)%P)
      ENDDO
! CV +1
...
!     ONLY BE INTIALISED BY THE AD USER SUBROUTINES AD_ALLVEC_SISYPHE
!
...
!
      IF(NADVAR.GT.0) THEN
!       THESE DIFFERENTIATED ARRAYS MUST EXIST BUT CAN BE EMPTY
        CALL BIEF_ALLVEC_IN_BLOCK(ADVAR,NADVAR,
     &                            1,'ADGRAD',IELMT,1,2,MESH)
      ENDIF
!
!> /!\ NOTE: PRIVE ARRAY HAVE TO BE HANDLED IN A MORE GENERIC WAY
      IF(NADVAR.GT.0) THEN
        DO I=1,NADVAR
          ...
          CALL AD_ALLVEC_SISYPHE(I,ADVAR%ADR(I)%P)
        ENDDO
      ENDIF
!
!###< SEB @ HRW
