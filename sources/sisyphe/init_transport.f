...
!
     ...
     & AVAIL,RATIO_SAND,ACLADM,UNLADM,KSP,KSR,KS,
...
!
      ...
      DOUBLE PRECISION,  INTENT(IN)  :: RATIO_SAND(NSICLA,NOMBLAY,NPOIN)
...
!           IF NON COHESIVE
     ...
     &        ACLADM, UNLADM,KSP,KSR,RATIO_SAND(I,1,1:NPOIN),
...
!
        ...
        ENDIF
!
!       CORRECTION JMH 15/01/2013: 21 CHANGED INTO 22
!
...
!
                CALL OS('X=Y     ', X=ZREF, Y=KSP)
!
                CMAX = MAX(CMAX,PARTHENIADES(1)/XWC(I)) ! to expand partheniades for layers
!
                IF(VITCE.GT.1.D-8.AND.TOCD(I).GT.1.D-8) THEN
                  DO J = 1, NPOIN
!                 FLUER
                  USTARP= SQRT(T8%R(J)/XMVE)
                  AAA= PARTHENIADES(1)* ! to expand partheniades for layers
