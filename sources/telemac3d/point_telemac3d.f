...
!
      IF(INCLUS(COUPLING,'SISYPHE').AND.INCLUS(COUPLING,'TOMAWAC')) THEN
...
!
      IF(SEDI) THEN
        ...
        CALL BIEF_ALLVEC_IN_BLOCK(WCHU,NCLASS,1,'WCHU  ',IELM3 ,
     &                            1,1,MESH3D)
! TBE ALSO ALLOCATE 1DV BLOCK (NPLANx11) FOR SET_DIF WORK ARRAYS
...
!        CALL BIEF_ALLVEC(1, FLUER, 'FLUER ', IELM2H, 1, 1,MESH2D)
        CALL BIEF_ALLVEC_IN_BLOCK(FLUER,NCLASS,1,'FLUER ',IELM2H ,
     &                            1,1,MESH2D)
! TBE not used, but called by impsed -- need removing 
        CALL BIEF_ALLVEC(1, PDEPO, 'PDEPO ', IELM2H, 1, 1,MESH2D) 
!        CALL BIEF_ALLVEC(1, FLUDP, 'FLUDP ', IELM2H, 1, 1,MESH2D)
        CALL BIEF_ALLVEC_IN_BLOCK(FLUDP,NCLASS,1,'FLUDP ',IELM2H ,
     &                            1,1,MESH2D)
!        CALL BIEF_ALLVEC(1, FLUDPT,'FLUDPT', IELM2H, 1, 1,MESH2D)
        CALL BIEF_ALLVEC_IN_BLOCK(FLUDPT,NCLASS,1,'FLUDPT',IELM2H ,
     &                            1,1,MESH2D)
...
!
      IF(MIXTE) THEN
        ...
        CALL BIEF_ALLVEC(1,WCS     ,'WCS   ',IELM3 ,1,1,MESH3D)
      ELSE
        ...
        CALL BIEF_ALLVEC(1,WCS     ,'WCS   ',0     ,1,0,MESH3D)
      ENDIF
!
!-----------------------------------------------------------------------
!
...
!
      ...
      CALL ADDBLO(VARSOR,ESOMT)           ! ESOMT 37 BED EVOLUTION DUE TO DUSPENSION
      CALL ADDBLO(VARSOR,TAIR)            ! K  38 AIR TEMPERATURE
!
!     VARIABLES 38+1 TO 38+NTRAC
!
      IF(NTRAC.GT.0) THEN
        DO I=1,NTRAC
!         SIZE OF TRAV2 MUST BE GREATER THAN 13+NTRAC
      ...
      ENDIF
!
!     LAYTHI: FROM 38+1+NTRAC   TO 38+NTRAC+NCOUCH
!
      IF(NCOUCH.GT.0.AND.SEDI) THEN
        DO I=1,NCOUCH
          CALL ADDBLO(VARSOR,LAYTHI%ADR(I)%P)
        ENDDO
      ENDIF
!
      CALL ADDBLO(VARSOR,PVSCO)           ! 39+NTRAC+NCOUCH
      CALL ADDBLO(VARSOR,PVSNCO)          ! 40+NTRAC+NCOUCH
!
!
...
!
!     THE FOLLOWING ADDRESS FOR TRACERS IS ADR_TRAC = 29
!     THIS NUMBER IS HARDCODED IN DECLARATIONS_TELEMAC3D
!
      IF(VARSO3%N.NE.ADR_TRAC-1) THEN
        WRITE(LU,*) 'POINT_TELEMAC3D:'
        IF(LNG.EQ.1) THEN
          WRITE(LU,*) 'ADR_TRAC DOIT VALOIR ',VARSO3%N+1
          WRITE(LU,*) 'DANS DECLARATIONS_TELEMAC3D'
        ENDIF
        IF(LNG.EQ.2) THEN
          WRITE(LU,*) 'ADR_TRAC MUST EQUAL ',VARSO3%N+1
          WRITE(LU,*) 'IN DECLARATIONS_TELEMAC3D'
        ENDIF
        CALL PLANTE(1)
        STOP
      ENDIF
